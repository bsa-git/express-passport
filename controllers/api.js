...
const graph = require('fbgraph');// NodeJs module to access the facebook graph api
const {LastFmNode} = require('lastfm');// Read and write to Last.fm with node.js
const tumblr = require('tumblr.js');// JavaScript client for the Tumblr API
const GitHub = require('@octokit/rest');// GitHub REST API client for Node.js
const Twit = require('twit');// Twitter API Client for node (REST & Streaming API)
const stripe = require('stripe')(process.env.STRIPE_SKEY);
const twilio = require('twilio');
const Linkedin = require('node-linkedin')(process.env.LINKEDIN_ID, process.env.LINKEDIN_SECRET, process.env.LINKEDIN_CALLBACK_URL);// LinkedIn 2.0 wrapper in Node.js
const clockwork = require('clockwork')({key: process.env.CLOCKWORK_KEY});
const paypal = require('paypal-rest-sdk');
const lob = require('lob')(process.env.LOB_KEY);
const ig = require('instagram-node').instagram();// Instagram
...
exports.getGithub = async (req, res, next) => {
    ...
    try {
        const {data: repo} = await github.repos.get({owner: 'bsa-git', repo: 'silex-mvc'});
...
exports.getInstagram = async (req, res, next) => {
    ...
    try {
        ...
        const searchByUsername = {}; //await userSearchAsync('bsa261257');
        const searchByUserId = {}; //await userAsync('2000082009');
